...
class QRCodeService
{
    ...
    private function findActiveCampaignForDcd(User $dcd, ?string $deviceFingerprint = null, ?array $geoData = null): ?\App\Models\Campaign
    {
        ...
        // Step 4: From campaigns with same deadline distance, prioritize oldest first
        ...
        $oldestCampaign = $sameDeadline->sortBy('created_at')->first();

        if (! $oldestCampaign) {
            return null;
        }

        // Step 5: If multiple with same age, find those with more budget
        $oldestCreatedAt = $oldestCampaign->created_at;
        $sameAge = $sameDeadline->filter(function ($campaign) use ($oldestCreatedAt) {
            return $campaign->created_at == $oldestCreatedAt;
        });

        $highestBudget = $sameAge->sortByDesc(function ($campaign) {
        ...
        // Step 6: If multiple with same budget and age, random pick
        $sameBudget = $sameAge->filter(function ($campaign) use ($highestBudget) {
